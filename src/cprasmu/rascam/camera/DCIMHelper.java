...
/**
 * {@code DCIMHelper} is used to provide DCIM-compliant information regarding
...
public class DCIMHelper {
	...
	//cpr change 28/09/13, was private
	public static File dcimDirectory = null;
	...
	static {
		 ...
		 */
		File firstDir = new File(dcimDirectory, "100RASCM");
		if (!firstDir.exists()) {
			if (!firstDir.mkdirs()) {
	...
	public static String getCurrentDirectory() {
		if (currentDir == null) {
			...
			if (dirs.length > 1) {
				...
				currentDir = new File(dcimDirectory, "100RASCM");
	...
	private static String getNameForNewItem(String prefix) {
		...
		if (nnnn >= 9999) {
			 ...
			 */
			log( "Unable to create new image name: already reached limit of 9999");
			return null;
		}
	...
	static void makeNewDirectory() {
		...
		if (nnn < 999) {
			nnn++;
			File newDir = new File(dcimDirectory, nfDirectory.format(nnn) + "RASCM");
			if (newDir.mkdirs()) {
				currentDir = newDir;
			} else {
